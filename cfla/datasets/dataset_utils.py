...
def split_labels_structured(labels, num_clients, classes_per_client):
    ...
    assert num_classes % classes_per_client == 0, (
        "Number of classes must be divisible by classes_per_client"
    )

    num_clusters = num_classes // classes_per_client
    assert num_clients % num_clusters == 0, (
        "Number of clients must be divisible by number of clusters"
    )
    ...
    for cluster_id in range(num_clusters):
        ...
        cluster_classes = sorted_classes[
            cluster_id * classes_per_client : (cluster_id + 1) * classes_per_client
        ]
...
def dirichlet_partition(labels, num_clients, alpha=0.1):
    ...
    try_cnt = 1
    while (
        min_size < least_samples and try_cnt <= 100
    ):  # we try 100 times to get the minimum sample size for each client.
        # if try_cnt > 1:
...
def pathological_partition(labels, num_clients, classes_per_client=2, seed=None):
    ...
    if num_clients > len(all_arrangements):
        selected_arrangements = [
            all_arrangements[i % len(all_arrangements)] for i in range(num_clients)
        ]
    ...
    for target, indices in class_indices.items():
        ...
        clients_with_target = [
            cid for cid, targets in enumerate(client_indices) if target in targets
        ]
...
def patho(labels, num_clients, classes_per_client=2, balance=True, least_samples=100):
    ...
    for i in range(num_classes):
        ...
        selected_clients = selected_clients[
            : int(np.ceil((num_clients / num_classes) * classes_per_client))
        ]
        ...
        if balance:
            ...
            num_samples = np.random.randint(
                max(num_per / 10, least_samples / num_classes), num_per, num_selected_clients - 1
            ).tolist()
        ...
        for client, num_sample in zip(selected_clients, num_samples):
            if client not in clients_data_indices.keys():
                ...
                clients_data_indices[client] = np.append(
                    clients_data_indices[client],
