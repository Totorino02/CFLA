...
class ParameterTree:
    ...
    def add_global_test_result(
        self, node_id, g_accuracy: float, global_test_loss_history: list[float]
    ):
        self.nodes[node_id].global_test_loss_history = global_test_loss_history
        self.nodes[node_id].global_accuracy = g_accuracy

    def add_child_cluster(
        self, parent_id: int, child_clients: set[int], model: torch.nn.Module
    ) -> int:
    ...
    def visualize_tree(self, filename):
        def _write_node(node: TreeNode, depth: int, file):
            indent = "  " * depth
            file.write(
                f"{indent}- Cluster {node.id} | Clients: {sorted(node.clients)} | Acc: {node.accuracy} | G_acc: {node.global_accuracy}\n"
            )
