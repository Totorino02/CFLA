...
from cfla.framework.common.utils import average_state_dict
from cfla.framework.server.serverbase import Server
...
class ServerHCFL(Server):
    def __init__(self, global_model, test_dataloader, args, **kwargs):
        ...
        # "embed": cluster on embedding updates only (HCFL); "full": cluster on all params (FL+HC)
        self.clustering_signal = args.get("clustering_signal", "embed")
    ...
    def pre_learning(self):
        ...
        # Collect update vectors from ALL clients after warm-up
        updates = []
        for client in self.clients:
            _, _, _ = client.train(self.global_model, round=self.initial_rounds, save_metrics=False)
            update_vector = []
            if self.clustering_signal == "full":
                # Cluster on all model parameters (FL+HC behaviour)
                param_pairs = zip(
                    client.local_model.parameters(),
                    self.global_model.parameters(),
                )
            else:
                # Cluster on embedding updates only (HCFL contribution)
                param_pairs = zip(
                    client.local_model.embed.parameters(),
                    self.global_model.embed.parameters(),
                )
            for new_param, old_param in param_pairs:
