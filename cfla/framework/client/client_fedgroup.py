"""
FedGroup client — offline clustered FL (Tan et al., 2022)

Standard local training (no regularization).
Returns full state_dict for FedAvg aggregation within the assigned cluster.
"""
...
class ClientFedGroup:
    """
    FedGroup client:
      - Receives the cluster model (or global model before clustering)
      - Trains locally with standard SGD (no regularization)
      - Returns full state_dict for server-side FedAvg aggregation
    """
    ...
    def train(
        ...
        **kwargs,
        ...
        """
        Standard local training from global_model.
        Returns:
