"""
FeSEM client — Federated learning with Self-organizing Expectation Maximization

Standard local training from the assigned cluster center (no regularization).
Returns full state_dict for per-cluster FedAvg aggregation.
"""
...
class ClientFeSEM:
    """
    FeSEM client:
      - Receives the assigned cluster center Ω_{k*}
      - Trains locally with standard SGD (no regularization)
      - Returns full state_dict for cluster FedAvg
    """
    ...
    @torch.no_grad()
    def get_flat_params(self, device=None) -> torch.Tensor:
        """Flattened local model parameters for L2 distance computation."""
        d = device or self.device
        return torch.cat([p.detach().to(d).flatten() for p in self.local_model.parameters()])
    ...
    def train(
        ...
        **kwargs,
        ...
        Returns:
          - full state_dict (cpu) for cluster FedAvg
          - last training loss
          - energy_consumed dict
        """
        self.local_model = type(cluster_model)().to(self.device)
        self.local_model.load_state_dict(cluster_model.state_dict(), strict=True)
        ...
        if energy_consumed:
            ...
            for k, e in energy_consumed.items():
                ...
                client_energy += e
            denom = float(self.get_flat_params().norm().item()) + 1e-9
