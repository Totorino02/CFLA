"""
CGPFL client — Cluster-Guided Personalized Federated Learning

Trains with a proximal regularization toward the assigned cluster center:
    L = L_sup + (μ/2) ||ω_i − Ω_{k*}||²

No global embedding aggregation (distinguishes CGPFL from LCFed/HCFL).
Returns full state_dict for per-cluster aggregation.
"""
...
class ClientCGPFL:
    """
    CGPFL client:
      - Receives assigned cluster center Ω_{k*}
      - Loss: L_sup + (μ/2)||ω_i − Ω_{k*}||²
    ...
    def __init__(self, client_id, dataset, args, output_dir, **kwargs):
        ...
        self.mu = float(args.get("mu", 1.0))  # cluster proximal coefficient
    ...
    def train(
        ...
        **kwargs,
        ...
        """
        Trains from global_model with proximal pull toward omega_cluster.
        Returns:
          - full state_dict (cpu)
          - last training loss
        ...
        # Frozen cluster center reference
        ...
        if energy_consumed:
            ...
            for k, e in energy_consumed.items():
                ...
                client_energy += e
            from cfla.framework.common.utils import flatten_params
