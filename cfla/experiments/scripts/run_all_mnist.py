...
def build_client_datasets_rotated_cifar10(base_seed: int, run: int):
    """
    Concept shift partition for CIFAR-10 via rotation.

    3 groups of clients, each with ALL 10 CIFAR-10 classes but a different
    rotation applied to every image:
      - Group 0 : 0°   (17 clients)
      - Group 1 : 90°  (17 clients)
      - Group 2 : 180° (16 clients)

    This creates a genuine feature-level concept shift that is invisible to
    label-based clustering methods but should be captured by embedding updates.
    The test loader uses the standard (0°) CIFAR-10 for global model reference;
    per-client evaluation uses each client's own rotated test split.
    """
    normalize = transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2470, 0.2435, 0.2616))
    rotations = [0, 90, 180]
    n_clients_per_group = [17, 17, 16]
    ...
    for angle, n_clients in zip(rotations, n_clients_per_group):
        if angle == 0:
            transform = transforms.Compose([transforms.ToTensor(), normalize])
        else:
        ...
        partitions = _split_into_n(subset, n_clients, seed=base_seed + run * (angle + 2))
        for p in partitions:
            client_datasets[cid] = p
            cid += 1
