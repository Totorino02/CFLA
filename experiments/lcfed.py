"""
Training script LCFed (arXiv:2501.01850) — même style que ton training FLHC.

Hypothèses:
- Tu as déjà les 2 classes:
    * framework.client.client_lcfed.ClientLCFed
    * framework.server.server_lcfed.ServerLCFed
...
for run in range(nb_runs):
    ...
    client_datasets = {}
    for i in range(10):
        client_datasets[i] = partitions_0_4[i]      # clients 0–9
    for i in range(10):
        client_datasets[i + 10] = partitions_5_9[i] # clients 10–19

    # Output dir
    stamp = datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
    output_dir = os.path.join("./RESULTS", f"result_lcfed_mnist_{stamp}")
    ...
    # - tu peux utiliser local_epochs OU local_steps (si ton ClientLCFed le supporte)
    client_args = {
        "local_epochs": 2,
    ...
    torch.save({"Omega_k": Omega_k, "Phi": Phi, "R": R}, os.path.join(output_dir, "lcfed_artifacts.pt"))

    print(f"Saved artifacts to: {os.path.join(output_dir, 'lcfed_artifacts.pt')}")