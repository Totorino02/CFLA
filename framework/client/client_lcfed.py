...
# -------------------------
# Example split model template
# You can replace by your own model as long as it exposes:
#   - model.embed : nn.Module  (ϕ)
#   - model.head  : nn.Module  (h)
# and forward(x) works.
# -------------------------
...
class ClientLCFed:
    """
    LCFed client:
      - receives global embedding Φ and its assigned cluster center Ω_{k*}
      - trains local full model ω_i with loss:
          L_sup + (μ/2)||ω_i - Ω_{k*}||^2 + (λ/2)||ϕ_i - Φ||^2
      - returns updated full params + low-rank vector z_i = M^T vec(ω_i)
    """
    ...
    def train(
        ...
        **kwargs
        ...
        Returns:
          ...
          - low_rank vector z_i = (vec(ω_i) @ M) (cpu) used for clustering
          - last loss (float)
        ...
        # Low-rank vector for clustering
        if self.M is None:
            raise RuntimeError("ClientLCFed: M not set. Server must broadcast M first.")
        vec = flatten_params(self.local_model, device=self.device)
        z = (vec @ self.M).detach().cpu()  # [D]
